      ...
      SELECT 
        ...
        (
          SELECT json_agg(json_build_object(
            ...
            'status', s.status,
            'end_date', s.end_date,
            'has_work_logged', EXISTS(SELECT 1 FROM daily_work_records d WHERE d.step_id = s.id OR (d.step_ids IS NOT NULL AND d.step_ids ? s.id::text)),
            'latest_work_date', (SELECT MAX(work_date) FROM daily_work_records d WHERE d.step_id = s.id OR (d.step_ids IS NOT NULL AND d.step_ids ? s.id::text))
